...
chrome-agent profiles clone work work-2     # "save as": work-2 starts from work's saved state (source must be stopped)
...
- **`profiles clone SOURCE NEW`** copies a stopped managed profile to a new name on this machine. A profile is the unit of sign-in and a fresh one always starts signed out, so this is how a new profile starts from an existing sign-in: keep one template profile with your Chrome sign-in and clone it. The clone gets everything Chrome saved (sessions, extensions, settings); Chrome's own lock files are not copied. This is the one place chrome-agent copies profile data; it does not inspect, interpret, decode or export any of it, and the copy stays on this machine where Chrome's credential store can open it. A site may still ask for a fresh login when it treats the clone as a new device. A profile containing unexpected symlinks is refused.
...
- **Exit codes:** `0` signed in, `2` a person must sign in, `3` error (probe threw, timed out, browser not reachable, bad or unknown arguments). Every outcome, including argument errors, prints one JSON object on stdout when not on a terminal.
- **`login-check`** opens its own background tab, never activates a window, never touches a tab it did not open, and closes its tab afterwards. If the site redirects to another host (an identity provider), the answer is `needs_login` and the probe is not run there. Sites often finish loading and *then* redirect from script, and Chrome delays a background tab's timers by a second or more, so that timing cannot be predicted. `login-check` therefore waits for the tab to hold one URL for a settle window (1 second) before probing, and checks again after the probe: if the tab has left the site by then, the probe's answer is discarded and the result is `needs_login`. The probe may occasionally run on the site's own page just before such a redirect; write it as a read (an authenticated `fetch`), which a signed-out session cannot pass. On macOS and Linux, checks on one profile run one at a time, so a check never has its browser stopped by another; on Windows there is no such lock (`flock` is unavailable), so run checks on one profile sequentially there. Given `--profile`/`--profile-dir` for a profile that is not running, it starts a headless browser, checks, and stops it again.
- **`login`** opens the site in a visible tab and polls only that tab, only while it is on the site's own host, so a sign-in ceremony on an identity provider's pages is never disturbed. It refuses an instance that was launched headless (exit `3`); for an instance started by an older chrome-agent this is read from the process command line, which is best effort and unavailable on Windows.

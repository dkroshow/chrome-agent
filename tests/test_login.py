...
def test_cli_check_on_stopped_profile_launches_checks_and_stops(tmp_path, site):
    ...
    assert os.path.isdir(tmp_path / "profiles" / "cli-site")

    bad = cli("login-check", "--profile", "cli-site", "--site", f"{site}/app")
    assert bad.returncode == 3 and "exactly one of --probe" in bad.stdout
    both = cli("login-check", "x-01", "--profile", "cli-site", "--site", f"{site}/app", "--probe-expr", "true")
    assert both.returncode == 3
